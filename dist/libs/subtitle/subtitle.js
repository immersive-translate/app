(()=>{var di=Object.defineProperty;var pi=(e,t)=>{for(var r in t)di(e,r,{get:t[r],enumerable:!0})};var C={BUILD_TIME:"2023-12-22T15:43:42.000Z",VERSION:"0.12.11",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"1",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=`${Pt.toTimeString(o.start)},${Pt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Ei=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Hr=Ae({name:_i,build:ki,detect:Ei,helper:Pt,parse:Ai});var qr="smi",kn={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
`).replace(/&nbsp;/g," ").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&")},Ci=(e,t)=>{if(t.format&&t.format!==qr)throw new Error(`Invalid format: ${t.format}`);let r=[],n=t.eol||`\r
...
`,n="";n+=`<SAMI>${r}`,n+=`<HEAD>${r}`,n+=`<TITLE>${t.title||""}</TITLE>${r}`,n+=`<STYLE TYPE="text/css">${r}`,n+=`<!--${r}`,n+=`P { font-family: Arial; font-weight: normal; color: white; background-color: black; text-align: center; }${r}`,n+=`.LANG { Name: ${t.langName||"English"}; lang: ${t.langCode||"en-US"}; SAMIType: CC; }${r}`,n+=`-->${r}`,n+=`</STYLE>${r}`,n+=`</HEAD>${r}`,n+=`<BODY>${r}`;for(let a=0;a<e.length;a++){let o=e[a];if(o.type!=="meta"){if(!o.type||o.type==="caption"){n+=`<SYNC Start=${o.start}>${r}`,n+=`  <P Class=LANG>${kn.htmlEncode(o.text||"")}${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`),n+=`<SYNC Start=${o.end}>${r}`,n+=`  <P Class=LANG>&nbsp;${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`);continue}t.verbose}}return n+=`</BODY>${r}`,n+=`</SAMI>${r}`,n},Li=e=>/<SAMI[^>]*>[\s\S]*<BODY[^>]*>/.test(e),zr=Ae({name:qr,build:Pi,detect:Li,helper:kn,parse:Ci});var Ni="srt",Lt={toMilliseconds:e=>{let t=/^\s*(\d{1,2}):(\d{1,2}):(\d{1,2})(?:[.,](\d{1,3}))?\s*$/.exec(e);if(!t)throw new Error(`Invalid time format: ${e}`);let r=parseInt(t[1]),n=parseInt(t[2]),a=parseInt(t[3]),o=t[4]?parseInt(t[4]):0;return r*3600*1e3+n*60*1e3+a*1e3+o},toTimeString:e=>{let t=Math.floor(e/1e3/3600),r=Math.floor(e/1e3/60%60),n=Math.floor(e/1e3%60),a=Math.floor(e%1e3);return`${(t<10?"0":"")+t}:${r<10?"0":""}${r}:${n<10?"0":""}${n},${a<100?"0":""}${a<10?"0":""}${a}`}},Oi=(e,t)=>{let r=[],n=t.eol||`\r
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=(o.index?o.index:a+1).toString()+n,r+=`${Lt.toTimeString(o.start)} --> ${Lt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Ri=e=>/\d+\r?\n\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*-->\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Gr=Ae({name:Ni,build:Mi,detect:Ri,helper:Lt,parse:Oi});var Di="sub",En=25,Ii=(e,t)=>{t.fps||=En;let r=t.fps>0?t.fps:En,n=[],a=t.eol||`\r
