(()=>{var ui=Object.defineProperty;var di=(e,t)=>{for(var r in t)ui(e,r,{get:t[r],enumerable:!0})};var C={BUILD_TIME:"2023-10-24T11:09:23.000Z",VERSION:"0.10.7",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"0",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=(o.index?o.index:a+1).toString()+n,r+=`${Lt.toTimeString(o.start)} --> ${Lt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Oi=e=>/\d+\r?\n\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*-->\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Gr=Ae({name:Li,build:Mi,detect:Oi,helper:Lt,parse:Ni});var Ri="sub",kn=25,Di=(e,t)=>{t.fps||=kn;let r=t.fps>0?t.fps:kn,n=[],a=t.eol||`\r
`,o=e.split(/\r?\n/g);for(let i=0;i<o.length;i++){let l=/^\{(\d+)\}\{(\d+)\}(.*)$/.exec(o[i]);if(l){let s={};s.type="caption",s.index=i+1,s.frame={start:parseInt(l[1]),end:parseInt(l[2]),count:parseInt(l[2])-parseInt(l[1])},s.start=Math.round(s.frame.start/r),s.end=Math.round(s.frame.end/r),s.duration=s.end-s.start;let u=l[3].split(/\|/g);s.content=u.join(a),s.text=s.content.replace(/\{[^}]+\}/g,""),n.push(s);continue}t.verbose}return n},Ii=(e,t)=>{let r=(t.fps||0)>0?t.fps:kn,n="",a=t.eol||`\r
