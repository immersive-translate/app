(()=>{var di=Object.defineProperty;var pi=(e,t)=>{for(var r in t)di(e,r,{get:t[r],enumerable:!0})};var C={BUILD_TIME:"2023-12-05T14:32:50.920Z",VERSION:"0.12.3",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"1",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=`${Pt.toTimeString(o.start)},${Pt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Ei=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Kr=Ae({name:_i,build:ki,detect:Ei,helper:Pt,parse:Ai});var zr="smi",kn={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
`).replace(/&nbsp;/g," ").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&")},Ci=(e,t)=>{if(t.format&&t.format!==zr)throw new Error(`Invalid format: ${t.format}`);let r=[],n=t.eol||`\r
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=(o.index?o.index:a+1).toString()+n,r+=`${Lt.toTimeString(o.start)} --> ${Lt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Di=e=>/\d+\r?\n\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*-->\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Gr=Ae({name:Ni,build:Mi,detect:Di,helper:Lt,parse:Oi});var Ri="sub",En=25,Ii=(e,t)=>{t.fps||=En;let r=t.fps>0?t.fps:En,n=[],a=t.eol||`\r
