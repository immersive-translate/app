(()=>{var ui=Object.defineProperty;var di=(e,t)=>{for(var r in t)ui(e,r,{get:t[r],enumerable:!0})};var A={BUILD_TIME:"2024-01-10T13:07:46.000Z",VERSION:"1.1.3",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"1",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=`${Lt.toTimeString(o.start)},${Lt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},ki=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Gr=Ae({name:Si,build:Ai,detect:ki,helper:Lt,parse:_i});var zr="smi",Cn={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
...
`,n="";n+=`<SAMI>${r}`,n+=`<HEAD>${r}`,n+=`<TITLE>${t.title||""}</TITLE>${r}`,n+=`<STYLE TYPE="text/css">${r}`,n+=`<!--${r}`,n+=`P { font-family: Arial; font-weight: normal; color: white; background-color: black; text-align: center; }${r}`,n+=`.LANG { Name: ${t.langName||"English"}; lang: ${t.langCode||"en-US"}; SAMIType: CC; }${r}`,n+=`-->${r}`,n+=`</STYLE>${r}`,n+=`</HEAD>${r}`,n+=`<BODY>${r}`;for(let a=0;a<e.length;a++){let o=e[a];if(o.type!=="meta"){if(!o.type||o.type==="caption"){n+=`<SYNC Start=${o.start}>${r}`,n+=`  <P Class=LANG>${Cn.htmlEncode(o.text||"")}${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`),n+=`<SYNC Start=${o.end}>${r}`,n+=`  <P Class=LANG>&nbsp;${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`);continue}t.verbose}}return n+=`</BODY>${r}`,n+=`</SAMI>${r}`,n},Pi=e=>/<SAMI[^>]*>[\s\S]*<BODY[^>]*>/.test(e),Vr=Ae({name:zr,build:Ei,detect:Pi,helper:Cn,parse:Ci});var Li="srt",Ot={toMilliseconds:e=>{let t=/^\s*(\d{1,2}):(\d{1,2}):(\d{1,2})(?:[.,](\d{1,3}))?\s*$/.exec(e);if(!t)throw new Error(`Invalid time format: ${e}`);let r=parseInt(t[1]),n=parseInt(t[2]),a=parseInt(t[3]),o=t[4]?parseInt(t[4]):0;return r*3600*1e3+n*60*1e3+a*1e3+o},toTimeString:e=>{let t=Math.floor(e/1e3/3600),r=Math.floor(e/1e3/60%60),n=Math.floor(e/1e3%60),a=Math.floor(e%1e3);return`${(t<10?"0":"")+t}:${r<10?"0":""}${r}:${n<10?"0":""}${n},${a<100?"0":""}${a<10?"0":""}${a}`}},Oi=(e,t)=>{let r=[],n=t.eol||`\r
`,a=e.split(/\r?\n\s*\n/g);for(let o=0;o<a.length;o++){let c=/^(\d+)\r?\n(\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?)\s*-->\s*(\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?)\r?\n([\s\S]*)$/.exec(a[o]);if(c){let l={};l.type="caption",l.index=parseInt(c[1]),l.start=Ot.toMilliseconds(c[2]),l.end=Ot.toMilliseconds(c[3]),l.duration=l.end-l.start;let s=c[4].split(/\r?\n/);l.content=s.join(n),l.text=l.content.replace(/<[^>]+>/g,"").replace(/\{[^}]+\}/g,"").replace(/>>[^:]*:\s*/g,""),r.push(l);continue}t.verbose}return r},Ni=(e,t)=>{let r="",n=t.eol||`\r
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=(o.index?o.index:a+1).toString()+n,r+=`${Ot.toTimeString(o.start)} --> ${Ot.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Mi=e=>/\d+\r?\n\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*-->\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Yr=Ae({name:Li,build:Ni,detect:Mi,helper:Ot,parse:Oi});var Di="sub",En=25,Ri=(e,t)=>{t.fps||=En;let r=t.fps>0?t.fps:En,n=[],a=t.eol||`\r
