(()=>{var ui=Object.defineProperty;var di=(e,t)=>{for(var r in t)ui(e,r,{get:t[r],enumerable:!0})};var C={BUILD_TIME:"2023-10-14T13:17:59.000Z",VERSION:"0.10.3",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"0",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`,i=e.split(/\r?\n\s*\n/);for(let c=0;c<i.length;c++){let s=/^\s*\[([^\]]+)\]\r?\n([\s\S]*)$/.exec(i[c]);if(s){let u=s[1],m=s[2].split(/\r?\n/);for(let p=0;p<m.length;p++){let g=m[p];if(/^\s*;/.test(g))continue;let h=/^\s*([^\s:]+):\s*(.*)$/.exec(g);if(h){if(u==="Script Info")if(r||(r={},r.type="meta",r.data={},a.push(r)),typeof r.data=="object"){let A=h[1].trim(),S=h[2].trim();r.data[A]=S}else throw new Error(`Invalid meta data: ${g}`);else if(u==="V4 Styles"||u==="V4+ Styles"){let A=h[1].trim(),S=h[2].trim();if(A==="Format")n=S.split(/\s*,\s*/g);else if(A==="Style"&&n){let f=S.split(/\s*,\s*/g),v={};v.type="style",v.data={};for(let E=0;E<n.length&&E<f.length;E++)v.data[n[E]]=f[E];a.push(v)}}else if(u==="Events"){let A=h[1].trim(),S=h[2].trim();if(A==="Format")n=S.split(/\s*,\s*/g);else if(A==="Dialogue"&&n){let f=S.split(/\s*,\s*/g),v={};v.type="caption",v.data={};for(let R=0;R<n.length&&R<f.length;R++)v.data[n[R]]=f[R];v.start=gt.toMilliseconds(v.data.Start),v.end=gt.toMilliseconds(v.data.End),v.duration=v.end-v.start,v.content=v.data.Text;let H=((R,B,_)=>R.split(B,_).join(B).length)(S,",",n.length-1)+1;v.content=S.substring(H),v.data.Text=v.content,v.text=v.content.replace(/\\N/g,o).replace(/\{[^}]+\}/g,""),a.push(v)}}}}}t.verbose}return a},_n=(e,t)=>{let r=t.eol||`\r
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=`${Pt.toTimeString(o.start)},${Pt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},Ei=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Kr=Ae({name:_i,build:Ai,detect:Ei,helper:Pt,parse:Si});var zr="smi",En={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
