(()=>{var Za=Object.defineProperty;var ei=(e,t)=>{for(var n in t)Za(e,n,{get:t[n],enumerable:!0})};var S={BUILD_TIME:"2023-09-17T09:17:18.000Z",VERSION:"0.9.5",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"0",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let o=0;o<e.length;o++){let a=e[o];if(!a.type||a.type==="caption"){n+=`${At.toTimeString(a.start)},${At.toTimeString(a.end)}${r}`,n+=a.text+r,n+=r;continue}t.verbose}return n},fi=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),Kr=Se({name:pi,build:mi,detect:fi,helper:At,parse:gi});var jr="smi",Cn={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
