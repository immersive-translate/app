(()=>{var ui=Object.defineProperty;var di=(e,t)=>{for(var r in t)ui(e,r,{get:t[r],enumerable:!0})};var A={BUILD_TIME:"2024-01-04T14:00:21.000Z",VERSION:"0.12.14",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",PROD_API:"1",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`;for(let a=0;a<e.length;a++){let o=e[a];if(!o.type||o.type==="caption"){r+=`${Lt.toTimeString(o.start)},${Lt.toTimeString(o.end)}${n}`,r+=o.text+n,r+=n;continue}t.verbose}return r},ki=e=>/\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*[,;]\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),jr=Ae({name:Si,build:Ai,detect:ki,helper:Lt,parse:_i});var qr="smi",En={htmlEncode:e=>e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<BR>"),htmlDecode:(e,t)=>e.replace(/<BR\s*\/?>/gi,t||`\r
`).replace(/&nbsp;/g," ").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&")},Ei=(e,t)=>{if(t.format&&t.format!==qr)throw new Error(`Invalid format: ${t.format}`);let r=[],n=t.eol||`\r
`,a=/<TITLE[^>]*>([\s\S]*)<\/TITLE>/i.exec(e);if(a){let s={};s.type="meta",s.name="title",s.data=a[1].replace(/^\s*/g,"").replace(/\s*$/g,""),r.push(s)}let o=/<STYLE[^>]*>([\s\S]*)<\/STYLE>/i.exec(e);if(o){let s={};s.type="meta",s.name="style",s.data=o[1],r.push(s)}let i=e.replace(/^[\s\S]*<BODY[^>]*>/gi,"").replace(/<\/BODY[^>]*>[\s\S]*$/gi,""),c=null,l=i.split(/<SYNC/gi);for(let s=0;s<l.length;s++){if(!l[s]||l[s].trim().length===0)continue;let u=`<SYNC${l[s]}`,m=/^<SYNC[^>]+Start\s*=\s*["']?(\d+)[^\d>]*>([\s\S]*)/i.exec(u);if(m){let p={};p.type="caption",p.start=parseInt(m[1]),p.end=p.start+2e3,p.duration=p.end-p.start,p.content=m[2].replace(/^<\/SYNC[^>]*>/gi,"");let g=!0,h=/^<P.+Class\s*=\s*["']?([\w-]+)(?: .*)?>([\s\S]*)/i.exec(p.content)||/^<P([^>]*)>([\s\S]*)/i.exec(p.content);if(h){let L=h[2].replace(/<P[\s\S]+$/gi,"");L=L.replace(/<BR\s*\/?>\s+/gi,n).replace(/<BR\s*\/?>/gi,n).replace(/<[^>]+>/g,""),L=L.replace(/^\s+/g,"").replace(/\s+$/g,""),g=L.replace(/&nbsp;/gi," ").replace(/\s+/g,"").length===0,p.text=En.htmlDecode(L,n)}!t.preserveSpaces&&g?t.verbose:r.push(p),c&&(c.end=p.start,c.duration=c.end-c.start),c=p;continue}t.verbose}return r},Ci=(e,t)=>{let r=t.eol||`\r
`,n="";n+=`<SAMI>${r}`,n+=`<HEAD>${r}`,n+=`<TITLE>${t.title||""}</TITLE>${r}`,n+=`<STYLE TYPE="text/css">${r}`,n+=`<!--${r}`,n+=`P { font-family: Arial; font-weight: normal; color: white; background-color: black; text-align: center; }${r}`,n+=`.LANG { Name: ${t.langName||"English"}; lang: ${t.langCode||"en-US"}; SAMIType: CC; }${r}`,n+=`-->${r}`,n+=`</STYLE>${r}`,n+=`</HEAD>${r}`,n+=`<BODY>${r}`;for(let a=0;a<e.length;a++){let o=e[a];if(o.type!=="meta"){if(!o.type||o.type==="caption"){n+=`<SYNC Start=${o.start}>${r}`,n+=`  <P Class=LANG>${En.htmlEncode(o.text||"")}${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`),n+=`<SYNC Start=${o.end}>${r}`,n+=`  <P Class=LANG>&nbsp;${t.closeTags?"</P>":""}${r}`,t.closeTags&&(n+=`</SYNC>${r}`);continue}t.verbose}}return n+=`</BODY>${r}`,n+=`</SAMI>${r}`,n},Pi=e=>/<SAMI[^>]*>[\s\S]*<BODY[^>]*>/.test(e),zr=Ae({name:qr,build:Ci,detect:Pi,helper:En,parse:Ei});var Li="srt",Nt={toMilliseconds:e=>{let t=/^\s*(\d{1,2}):(\d{1,2}):(\d{1,2})(?:[.,](\d{1,3}))?\s*$/.exec(e);if(!t)throw new Error(`Invalid time format: ${e}`);let r=parseInt(t[1]),n=parseInt(t[2]),a=parseInt(t[3]),o=t[4]?parseInt(t[4]):0;return r*3600*1e3+n*60*1e3+a*1e3+o},toTimeString:e=>{let t=Math.floor(e/1e3/3600),r=Math.floor(e/1e3/60%60),n=Math.floor(e/1e3%60),a=Math.floor(e%1e3);return`${(t<10?"0":"")+t}:${r<10?"0":""}${r}:${n<10?"0":""}${n},${a<100?"0":""}${a<10?"0":""}${a}`}},Ni=(e,t)=>{let r=[],n=t.eol||`\r
